...
def main(argv):
    argparser = argparse.ArgumentParser(description='Split flac file using cue sheet.')
    argparser.add_argument('--version', action='version', version='%(prog)s 1.0')
    argparser.add_argument('-id', '--input_dir', metavar='Input Directory', type=str, nargs=1, required=True,
                           help='Input starting directory; we will traverse this for cue files')
    argparser.add_argument('-od', '--output_dir', metavar='Output Directory', type=str, nargs=1, required=True,
                           help='single output path for where the results will end up')
    args = argparser.parse_args()

    
    # glob for all cue file paths. Note: '**', is recursive in python 3.5
    for i_cue_file in glob.glob(os.path.join(args.input_dir[0], '**', '*.cue'), recursive=True):
        ...
        if not os.path.isfile(i_cue_file):
            ...
            new_f_path = os.path.join(args.output_dir[0], cs.tags['ALBUM'] )
